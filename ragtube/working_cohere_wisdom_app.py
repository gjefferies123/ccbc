#!/usr/bin/env python3
"""Working Biblical Wisdom App with current Cohere SDK."""
...
class WorkingCohereWisdomBot:
    """Biblical guidance bot using current Cohere SDK properly."""
    ...
    def _init_clients(self):
        ...
        try:
            ...
            logger.info("✅ Working Cohere Biblical Wisdom App ready")
    ...
    def get_biblical_guidance(self, 
        ...
        try:
            ...
            # Step 3: Generate biblical guidance using current Cohere API
    ...
    def _generate_biblical_response_current(self, question: str, sermon_context: str) -> str:
        """Generate biblical guidance using current Cohere API properly."""
        
        # Prepare the full message for the current API
        full_message = f"""You are a helpful Christian guidance app that provides practical biblical wisdom. Give clear, practical advice based on biblical principles, use the provided sermon content as supporting wisdom, address real-world situations with biblical worldview, be encouraging but honest about challenges, suggest concrete next steps or actions, and reference relevant scripture when helpful.
...
Please provide practical biblical guidance for this question:"""
        ...
        try:
            # Use current Cohere chat API structure
            response = self.cohere_client.chat(
                message=full_message,  # Single message parameter, not messages array
                max_tokens=400,
                temperature=0.6
            )
            ...
            logger.warning(f"Cohere chat failed: {e}")
            return self._fallback_biblical_response(question)
    ...
    def _generate_general_biblical_response(self, question: str) -> Dict[str, Any]:
        ...
        message = f"""You are a helpful Christian guidance app. Provide practical biblical wisdom for: "{question}"
        ...
        try:
            ...
            if hasattr(response, 'text'):
                ...
                if isinstance(response.message.content, list):
                    ...
                    guidance_text = response.message.content.strip()
            else:
...
def main():
    """Test the working Cohere biblical wisdom app."""
    print("📖 WORKING COHERE BIBLICAL WISDOM APP")
    print("=" * 50)
    
    try:
        wisdom_bot = WorkingCohereWisdomBot()
        ...
        print(f"\n✅ Working Cohere Biblical Wisdom App is functional!")
        return True
